...
if not st.session_state['autenticado']:
    ...
    # 1. LIGA O MOTOR (Primeira coisa a fazer para evitar erros)
    ...
    with col_meta:
        with st.popover("⚙️ Ajustar Meta"):
            nova_meta = st.number_input("Sua Meta Mensal (R$)", value=meta_atual, step=100.0)
            if st.button("Salvar Nova Meta"):
                engine.salvar_meta(nova_meta)
    ...
    m2.metric("Sua Meta", f"R$ {meta_atual:.2f}")
    m3.metric("Falta para Meta", f"R$ {max(meta_atual - lucro_total, 0.0):.2f}")
    
    st.progress(progresso)
    st.markdown("---")

    # 5. FORM DE LANÇAMENTO
    with st.expander("➕ Lançar Novo Turno"):
        with st.form("form_turno", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar Turno"):
                if bruto > 0 and km > 0:
                    dados_calculados = engine.calcular_turno(bruto, km, preco_gas_input, consumo_input)
                    from datetime import datetime
